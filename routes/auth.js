...
router.post('/login', (req, res) => {
  ...
  if (!email || !password) return res.status(400).json({ error: 'Login and password required' });

  const login = email.toLowerCase().trim();
  const user = db.get('users', u => {
    if (!u.is_active) return false;
    return [u.email, u.login_id, u.employee_id]
      .filter(Boolean)
      .some(value => String(value).toLowerCase().trim() === login);
  });
